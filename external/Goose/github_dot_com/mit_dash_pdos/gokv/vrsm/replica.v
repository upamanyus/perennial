        ...
        (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
          ...
          then
            ...
            then
              ...
              sync.Cond__Wait (struct.loadF Server "committedNextIndex_cond" "s");;
              Continue)));;
        sync.Mutex__Unlock (struct.loadF Server "mu" "s");;
        "reply")).
...
Definition Server__IncreaseCommitIndex: val :=
  rec: "Server__IncreaseCommitIndex" "s" "newCommittedNextIndex" :=
    ...
    then
      ...
      sync.Cond__Broadcast (struct.loadF Server "committedNextIndex_cond" "s")
    else #());;
    sync.Mutex__Unlock (struct.loadF Server "mu" "s");;
    #().
  ...
  rec: "Server__Apply" "s" "op" :=
    ...
    then
      ...
      then
        ...
        let: "clerks_inner" := SliceGet (slice.T ptrT) "clerks" ((machine.RandomUint64 #()) `rem` (slice.len "clerks")) in
  ...
  rec: "Server__sendIncreaseCommitThread" "s" :=
    ...
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      ...
      (for: (λ: <>, (~ (struct.loadF Server "isPrimary" "s")) || ((slice.len (SliceGet (slice.T ptrT) (struct.loadF Server "clerks" "s") #0)) = #0)); (λ: <>, Skip) := λ: <>,
        sync.Cond__Wait (struct.loadF Server "isPrimary_cond" "s");;
        Continue);;
...
Definition Server__ApplyAsBackup: val :=
  rec: "Server__ApplyAsBackup" "s" "args" :=
    ...
    (for: (λ: <>, (((struct.loadF ApplyAsBackupArgs "index" "args") > (struct.loadF Server "nextIndex" "s")) && ((struct.loadF Server "epoch" "s") = (struct.loadF ApplyAsBackupArgs "epoch" "args"))) && (~ (struct.loadF Server "sealed" "s"))); (λ: <>, Skip) := λ: <>,
      ...
      (if: (~ "ok")
      then
        let: "cond" := sync.NewCond (struct.loadF Server "mu" "s") in
...
Definition Server__BecomePrimary: val :=
  rec: "Server__BecomePrimary" "s" "args" :=
    ...
    then
      ...
      sync.Cond__Signal (struct.loadF Server "isPrimary_cond" "s");;
...
Definition MakeServer: val :=
  rec: "MakeServer" "sm" "confHosts" "nextIndex" "epoch" "sealed" :=
    ...
    struct.storeF Server "committedNextIndex_cond" "s" (sync.NewCond (struct.loadF Server "mu" "s"));;
    struct.storeF Server "isPrimary_cond" "s" (sync.NewCond (struct.loadF Server "mu" "s"));;
    "s".
