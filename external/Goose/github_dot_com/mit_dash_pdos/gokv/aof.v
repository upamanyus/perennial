...
Definition CreateAppendOnlyFile: val :=
  rec: "CreateAppendOnlyFile" "fname" :=
    ...
    struct.storeF AppendOnlyFile "mu" "a" (struct.alloc sync.Mutex (zero_val (struct.t sync.Mutex)));;
    struct.storeF AppendOnlyFile "lengthCond" "a" (sync.NewCond (struct.loadF AppendOnlyFile "mu" "a"));;
    struct.storeF AppendOnlyFile "oldDurableCond" "a" (sync.NewCond (struct.loadF AppendOnlyFile "mu" "a"));;
    struct.storeF AppendOnlyFile "durableCond" "a" (sync.NewCond (struct.loadF AppendOnlyFile "mu" "a"));;
    struct.storeF AppendOnlyFile "closedCond" "a" (sync.NewCond (struct.loadF AppendOnlyFile "mu" "a"));;
    Fork (sync.Mutex__Lock (struct.loadF AppendOnlyFile "mu" "a");;
          ...
          (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
            ...
            then
              ...
              then
                grove__ffi.FileAppend "fname" (struct.loadF AppendOnlyFile "membuf" "a");;
