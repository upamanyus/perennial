...
Definition AsyncFile__flushThread: val :=
  rec: "AsyncFile__flushThread" "s" :=
    ...
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      ...
      then
        grove__ffi.FileWrite (struct.loadF AsyncFile "filename" "s") (struct.loadF AsyncFile "data" "s");;
        ...
        then
          ...
          sync.Mutex__Unlock (struct.loadF AsyncFile "mu" "s");;
          grove__ffi.FileWrite (struct.loadF AsyncFile "filename" "s") "data";;
          sync.Mutex__Lock (struct.loadF AsyncFile "mu" "s");;
...
Definition MakeAsyncFile: val :=
  rec: "MakeAsyncFile" "filename" :=
    ...
    struct.storeF AsyncFile "mu" "s" (struct.alloc sync.Mutex (zero_val (struct.t sync.Mutex)));;
    struct.storeF AsyncFile "indexCond" "s" (sync.NewCond (struct.loadF AsyncFile "mu" "s"));;
    struct.storeF AsyncFile "durableIndexCond" "s" (sync.NewCond (struct.loadF AsyncFile "mu" "s"));;
    struct.storeF AsyncFile "closedCond" "s" (sync.NewCond (struct.loadF AsyncFile "mu" "s"));;
    struct.storeF AsyncFile "filename" "s" "filename";;
    struct.storeF AsyncFile "data" "s" (grove__ffi.FileRead "filename");;
