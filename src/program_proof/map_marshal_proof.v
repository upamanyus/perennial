...
Proof.
  ...
  iDestruct (own_slice_small_nil byteT 1 Slice.nil) as "HH".
  { done. }
...
Lemma wp_byteMapGet mptr m (k:u64) :
  ...
  {{{
        sl, RET (slice_val sl);
        readonly (own_slice_small sl byteT 1 (default [] (m !! k))) ∗
        own_byte_map mptr m
  }}}
.
Proof.
...
Proof.
  ...
  iNamed "Hmap".
  iMod (readonly_alloc (own_slice_small sl byteT 1 v) with "[Hsl]") as "#Hsl".
  { done. }
