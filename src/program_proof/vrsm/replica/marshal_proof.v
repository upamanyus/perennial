...
Lemma wp_Encode (args_ptr:loc) (args:C) :
  {{{
    ...
    EncodeApplyAsBackupArgs #args_ptr
  {{{
...
Definition own args_ptr args : iProp Σ :=
  ∃ state_sl,
  "Hargs_epoch" ∷ args_ptr ↦[SetStateArgs :: "Epoch"] #args.(epoch) ∗
...
Definition own args_ptr args : iProp Σ :=
  "Hargs_epoch" ∷ args_ptr ↦[GetStateArgs :: "Epoch"] #args.(epoch)
  .
...
Definition own reply_ptr reply : iProp Σ :=
  ∃ state_sl,
  "Hreply_epoch" ∷ reply_ptr ↦[GetStateReply :: "Err"] #reply.(err) ∗
...
Lemma wp_Encode (args_ptr:loc) (args:C) :
  {{{
    ...
    EncodeBecomePrimaryArgs #args_ptr
  {{{
        enc enc_sl, RET (slice_val enc_sl);
        ⌜has_encoding enc args⌝ ∗
        own_slice enc_sl byteT 1 enc ∗
